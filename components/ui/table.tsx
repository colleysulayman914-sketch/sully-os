    ...
    {composeRenderProps(children, (children, { allowsSorting }) => (
      <div className="flex items-center">
        <Group
          ...
          tabIndex={-1}
          className={cn(
            "flex h-10 min-h-[44px] flex-1 items-center gap-1 overflow-hidden rounded-md px-4",
            allowsSorting &&
...
const Row = <T extends object>({ className, ...props }: RowProps<T>) => (
  <AriaRow
    className={composeRenderProps(className, (className) =>
      cn(
        "border-b -outline-offset-2 transition-colors data-[hovered]:bg-muted/50 data-[selected]:bg-muted data-[focus-visible]:outline-ring min-h-[44px]",
        className
      )
    )}
    {...props}
